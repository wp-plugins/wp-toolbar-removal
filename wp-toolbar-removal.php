...
Version: 2012.0609-BUGFIX.0651-DEVELOPMENTAL
Author: sLa
...
//}
function wptbr_rsb(){
echo'<style type="text/css">body.admin-bar,body.admin-bar #wpcontent,body.admin-bar #adminmenu,body.admin-bar #wpadminbar,body.admin-bar #quicklinks{padding-top:0px !important}</style>';
}
add_action('admin_print_styles','wptbr_rsb',21);
function wptbr_rsf(){
echo'<style type="text/css">html{margin-top:0px !important}* html body{margin-top:0px !important}</style>';
}
...
if(!($current_user instanceof WP_User))return;echo''.$current_user->display_name.''?><?php if(is_multisite()&&is_super_admin()){if(!is_network_admin()){echo' | <a href="'.network_admin_url(). '">'. __('Network Admin'). '</a>';
}
else{echo' | <a href="' .get_dashboard_url(get_current_user_id()). '">'.__('Site Admin') . '</a>';}}?> | <?php echo'<a href="' .wp_logout_url(home_url()). '">'.__('Log Out'). '</a>'?></div></td><td width="8"></td></tr></table><?php }
add_action('in_admin_header','wptbr_hdr');

remove_action('init','wp_admin_bar_init');
//remove_filter('init','wp_admin_bar_init');
//remove_action('init','wp_toolbar_init');
//remove_action('init','wp_tool_bar_init');
//remove_action('init','admin_bar_init');
//remove_action('init','toolbar_init');
//remove_action('init','tool_bar_init');

//add_filter('init','wp_admin_bar_init');

//remove_filter('admin_bar_menu','nacin_promote_network_admin_in_toolbar',25);
...
//remove_action('admin_head','nacin_promote_network_admin_in_toolbar',25);
//remove_action('admin_bar_menu','nacin_promote_network_admin_in_toolbar',25);
//remove_action('personal_options','_admin_bar_pref');
//remove_action('personal_options','_get_admin_bar_pref');

//remove_action('admin_head','admin_bar_menu');
...
//wp_deregister_style('admin_bar_menu');
//wp_deregister_style('nacin_promote_network_admin_in_toolbar');
//wp_deregister_script('nacin_promote_network_admin_in_toolbar');
//wp_deregister_script('admin-bar');
//wp_deregister_script('admin_bar_menu');
...
function wptbr_footer_log(){
echo"\n<!--Plugin ToolBar Removal 2012.0609-BUGFIX.0651-DEVELOPMENTAL Active-->";
}
